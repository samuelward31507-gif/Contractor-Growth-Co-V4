...
import { updateInstantLeadFollowupConfig } from "../actions";
import { BusinessHoursToggleField } from "./business-hours-toggle-field";

/**
 * Automation Configuration V2.2 - only ever rendered for org admins (the
 * page checks membership.role before rendering this at all, same gate as
 * EnableToggle/AppointmentReminderConfigForm). `initialRespectBusinessHours`
 ...
 */
export function InstantLeadFollowupConfigForm({
  ...
  function commit(value: boolean) {
    ...
    startTransition(async () => {
      const result = await updateInstantLeadFollowupConfig(value);
      if (result.error) {
