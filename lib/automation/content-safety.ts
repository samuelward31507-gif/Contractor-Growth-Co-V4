...
const SCHEDULING_CLAIM_PATTERN =
  /\b(?:will|can|will be)\s+(?:arrive|be there|be over|get there|show up)\b|\bon (?:our|my) way\b|\beta\b|\bscheduled\s+(?:for|at|on)\b|\bbooked\s+(?:you|for|an? (?:appointment|technician))\b|\btechnician\s+is\s+(?:on|booked|assigned|en route)\b/i;
