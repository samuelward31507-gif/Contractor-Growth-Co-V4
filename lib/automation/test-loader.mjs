...
export async function resolve(specifier, context, nextResolve) {
  ...
  // A bare package subpath with no extension and no "exports" map entry
  // (e.g. "next/server", which resolves fine under Node's CJS require() -
  // see next/server.js on disk - but not under plain ESM resolution, which
  // never auto-appends an extension the way CJS does) fails here with
  // ERR_MODULE_NOT_FOUND. Retrying once with a literal ".js" appended is a
  // generic fix for any such package export, not a hardcoded special case
  // for next/server specifically.
  try {
